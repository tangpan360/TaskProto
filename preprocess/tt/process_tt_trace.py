...
def process_tt_traces():
    print("=== Start processing TT trace data ===")
    ...
    # Cache all span records: {service_name: [span_records]}
    # For memory usage, we could process in batches; for now we try one pass and optimize if needed.
    service_spans = {} 
    ...
    for folder in tqdm(exp_folders, desc="Parsing trace files"):
        ...
        # A trace object contains traceID, spans, processes, etc.
        ...
        for trace_obj in traces_data:
            ...
            for span in spans:
                ...
                # Get spanID and references (parentID).
                span_id = span.get('spanID', '')
                parent_id = ''
                references = span.get('references', [])
                for ref in references:
                    # Only handle CHILD_OF relations; other relations (e.g., FOLLOWS_FROM) are ignored.
                    if ref.get('refType') == 'CHILD_OF':
                        parent_id = ref.get('spanID', '')
                        break
                # If there is no CHILD_OF reference, parent_id remains empty (root span).
    ...
    print("=== TT trace processing done ===")
