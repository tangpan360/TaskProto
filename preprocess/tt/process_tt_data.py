...
# ⚠️ Important: must match window_size in process_tt_label.py
NUM_TIME_STEPS = 20  # seconds
...
def preload_all_data():
    ...
    Note: TT timestamps are already in seconds (int/float). Unlike Gaia, do NOT divide by 10**6
    and do NOT multiply by 1000.
    """
    print("=" * 50)
    print("Preloading all TT data into memory...")
    print("=" * 50)
...
def _process_log_for_sample(st_time, ed_time, normalize=True):
    ...
    for instance_idx, instance_name in enumerate(SERVICES):
        ...
        # TT log timestamps are float seconds
        mask = (df['timestamp'] >= st_time) & (df['timestamp'] < ed_time)
        sample_data = df[mask]
